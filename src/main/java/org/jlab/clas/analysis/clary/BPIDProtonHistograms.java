...
public class BPIDProtonHistograms {
	...
	h_pr_beta_time.add( new H1F("h_"+s_run_number+"_pr_betatime_cutlvl"+Integer.toString(i),"h_"+s_run_number+"_pr_betatime_cutlvl"+Integer.toString(i), 100, min_b, max_b ) );
	h_pr_beta_mntm.add( new H1F("h_"+s_run_number+"_pr_betamntm_cutlvl"+Integer.toString(i),"h_"+s_run_number+"_pr_betamntm_cutlvl"+Integer.toString(i), 100, min_b, max_b ) );
	h2_pr_tof.add( new H2F("h2_"+s_run_number+"_pr_tof_cutlvl"+Integer.toString(i),"h2_"+s_run_number+"_pr_tof_cutlvl"+Integer.toString(i), 100, min_p, max_p, 100, min_tof, max_tof ) );
	
	h2_pr_deltimep.add( new H2F("h2_"+s_run_number+"_pr_deltimep_cutlvl"+Integer.toString(i),"h2_"+s_run_number+"_pr_deltimep_cutlvl"+Integer.toString(i),100, min_p, max_p, 100, min_delt, max_delt ));
	...
	for( int s = 0; s < sector; s++ ){	    
		...
		for( int c = 0; c < max_cuts; c++ ){
		    ...
		    mh_temp.get(p).add(new H2F("h_"+s_run_number+"_pr_"+Integer.toString(s)+"_"+Integer.toString(p)+"_cutlvl"+Integer.toString(c),"h_"+s_run_number+"_pr_"+Integer.toString(s)+"_"+Integer.toString(p)+"_cutlvl"+Integer.toString(c), 200, 0.0, 10.5, 200, -10.0, 10.0) );
		}
	    }
	}				   
    }
	...
	F1D f_beta_cuttop = new F1D("f_betacut_top","x/sqrt(x*x + [0]*[0])",0.1,5.5);
	f_beta_cuttop.setParameter(0,PhysicalConstants.mass_proton*PhysicalConstants.mass_proton);	
	f_beta_cuttop.setLineColor(4);
